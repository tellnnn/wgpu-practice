...
impl State {
    async fn new(window: &Window) -> Self {
        ...
        let diffuse_bytes2 = include_bytes!("image2.png");
        let diffuse_texture2 = texture::Texture::from_bytes(&device, &queue, diffuse_bytes2, "image.2png").unwrap();
    ...
    fn input(&mut self, event: &WindowEvent) -> bool {
        match event {
            WindowEvent::KeyboardInput {
                ...
                self.use_another = *state == ElementState::Pressed;
                true
            },
            _ => false,
        }
    ...
    fn render(&mut self) -> Result<(), wgpu::SwapChainError> {
...

        {
            ...
            let data = if self.use_another {
                &self.diffuse_bind_group2
            } else {
                &self.diffuse_bind_group
            };

            render_pass.set_bind_group(0, data, &[]);
